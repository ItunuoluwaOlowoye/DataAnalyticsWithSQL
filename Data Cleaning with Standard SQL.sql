...
SELECT  
    *
FROM 
    `elite-replica-332817.customer_data.furniture_store_transactions`
...
SELECT 
    CAST (purchase_price AS FLOAT64) AS clean_purchase_price,
    product_code
FROM 
    `elite-replica-332817.customer_data.furniture_store_transactions`
ORDER BY 
...
SELECT 
    date, CAST(purchase_price AS FLOAT64) AS clean_purchase_price
FROM
    `elite-replica-332817.customer_data.furniture_store_transactions`
WHERE
...
--Finally, we want to get a list of names for all products that were sold
SELECT 
    product
FROM
     `elite-replica-332817.customer_data.furniture_store_transactions`
